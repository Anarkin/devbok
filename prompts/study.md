...
- Interview Q&A items are collapsed `<details>` self-quiz blocks: the question visible, the model answer hidden until clicked; a 1px `--border` box, a bold summary, and the marker in `--accent`. Answers are spoken-ready (3–6 sentences), carry lead-level nuance and name the trade-offs — not textbook definitions.
